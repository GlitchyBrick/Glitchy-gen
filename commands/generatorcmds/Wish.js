...
module.exports = class MinecraftCommand extends Command {
  ...
  run(message) {
    ...
    var Wish_File_Buffer = fs.readFileSync("./Wish.txt");
    var Wish_File_Buffer_To_String = Wish_File_Buffer.toString();
    var Wish_File_Buffer_Split_Lines = Wish_File_Buffer_To_String.split(
      "\n"
    );
    
    var lines = Wish_File_Buffer_Split_Lines.length-1
    ...
    if (lines === 0) {
        ...
        mentionHook.send(
          message.author.toString() +
            " Has Been Listed For DM Delivery For: Wish" +
    ...
    function procced() {
      fs.readFile("./Wish.txt", function read(err, data) {
        if (err) {
          throw err;
        }
        ...
        fs.writeFile("./Wish.txt", updatedData, err => {
          if (err) throw err;
        });
      });
    }
     ...
     function genlink(insidee) {
      ...
      exampleEmbed.setTitle("Wish Delivery:");
