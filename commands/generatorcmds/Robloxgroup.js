...
module.exports = class CrunchyrollCommand extends Command {
  ...
  run(message) {
    ...
    var lines = Robloxg_File_Buffer_Split_Lines.length - 1;
   ...
   if (lines === 0) {
        ...
        mentionHook.send(
          message.author.toString() +
            " Has Been Listed For DM Delivery For: Roblox Group" +
    ...
    function procced() {
      fs.readFile("./Robloxgroup.txt", function read(err, data) {
        if (err) {
          throw err;
        }
        ...
        fs.writeFile("./Robloxgroup.txt", updatedData, err => {
          if (err) throw err;
        });
      });
    }
     ...
     function genlink(insidee) {
      ...
      exampleEmbed.setTitle("Roblox Group Delivery:");
