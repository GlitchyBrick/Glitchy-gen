...
module.exports = class MinecraftCommand extends Command {
  ...
  run(message) {
    ...
    var lines = Minecraft_File_Buffer_Split_Lines.length - 1;
    ...
    if (lines === 0) {
        ...
        mentionHook.send(
          message.author.toString() +
            " Has Been Listed For DM Delivery For: Minecraft" +
    ...
    function procced() {
      fs.readFile("./minecraft.txt", function read(err, data) {
        if (err) {
          throw err;
        }
        ...
        fs.writeFile("./minecraft.txt", updatedData, err => {
          if (err) throw err;
        });
      });
    }
     ...
     function genlink(insidee) {
      ...
      exampleEmbed.setTitle("Minecraft Delivery:");
