...
module.exports = class MinecraftCommand extends Command {
  ...
  run(message) {
    ...
    var Origin_File_Buffer = fs.readFileSync("./origin.txt");
    var Origin_File_Buffer_To_String = Origin_File_Buffer.toString();
    var Origin_File_Buffer_Split_Lines = Origin_File_Buffer_To_String.split(
      "\n"
    );

    var lines = Origin_File_Buffer_Split_Lines.length - 1;
    ...
    if (lines === 0) {
        ...
        mentionHook.send(
          message.author.toString() +
            " Has Been Listed For DM Delivery For: Origin" +
    ...
    function procced() {
      fs.readFile("./origin.txt", function read(err, data) {
        if (err) {
          throw err;
        }
        ...
        fs.writeFile("./origin.txt", updatedData, err => {
          if (err) throw err;
        });
      });
    }
     ...
     function genlink(insidee) {
      ...
      exampleEmbed.setTitle("Origin Delivery:");
