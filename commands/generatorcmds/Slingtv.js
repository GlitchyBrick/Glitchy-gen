...
module.exports = class SlingtvCommand extends Command {
  constructor(client) {
    super(client, {
      name: "slingtv",
      group: "generatorcmds",
  ...
  run(message) {
    ...
    var Slingtv_File_Buffer = fs.readFileSync("./slingtv.txt");
    var Slingtv_File_Buffer_To_String = Slingtv_File_Buffer.toString();
    var Slingtv_File_Buffer_Split_Lines = Slingtv_File_Buffer_To_String.split(
      "\n"
    );

    var lines = Slingtv_File_Buffer_Split_Lines.length - 1;
    ...
    if (lines === 0) {
        ...
        mentionHook.send(
          message.author.toString() +
            " Has Been Listed For DM Delivery For: SlingTV" +
    ...
    function procced() {
      fs.readFile("./slingtv.txt", function read(err, data) {
        if (err) {
          throw err;
        }
        ...
        fs.writeFile("./slingtv.txt", updatedData, err => {
          if (err) throw err;
        });
      });
    }
     ...
     function genlink(insidee) {
      ...
      exampleEmbed.setTitle("Sling TV Delivery:");
